...
状态：Done（2026-07-19）。CAP-01 至 CAP-19 已完成；后续业务增强和外部上游风险不再作为 breaking migration 未完成项。
...
| CAP-19 | Docker、GitHub Actions、DockerHub、Argo CD、K8S | `platform` | Java API、Python quant 独立 Deployment；PostgreSQL；NetworkPolicy | 镜像签名/扫描、Secret 引用、迁移 Job、Argo Synced/Healthy、回滚演练 | Completed |
...
- `services/backend/gradlew clean test bootJar`、Python Ruff/mypy/pytest、前端 lint/test/build 全部通过。
...
- `apps/web/src` 不包含 `/api/v1`，生产部署不包含 Python Web/Worker 命令和 SQLite 数据库路径。
...
- 回滚演练能恢复上一 Java 镜像与 PostgreSQL/Longhorn 事故快照；冻结 Python 入口已退役并移入独立归档，不再作为生产回滚路径。Java 新库不做自动反向写回。
