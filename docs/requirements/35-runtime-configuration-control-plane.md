...
- 外部依赖故障不得导致定时任务无界堆积；同一 `schedule_id` 已有 `PENDING` 或 `CLAIMED` 实例时，不创建重叠实例，但仍推进下一次计划时间并保留既有任务历史。
- Worker 必须在运行期间周期恢复过期租约，不能只在进程启动时恢复；滚动发布期间旧 Worker 遗留的租约必须在一个恢复周期内重新进入重试或终态。
- 代理 readiness 必须区分本地服务可管理与目标出口可用；节点只有通过目标服务的 HTTP/响应体探测后才能进入轮转，全部失败时出口 fail closed，但控制面仍可接收热更新。
...
11. 信源在线测试使用刚保存的 endpoint、代理和凭据；控制面以带独立幂等域的持久化 `INGESTION` 任务返回 `202`，UI 轮询短请求并从采集运行读取结构化结果，外部依赖耗时不得占用同步 HTTP 请求直至 Cloudflare 超时。
12. 连续 Firecrawl 请求建立独立代理连接并按节点轮询；线上出口采样不能持续固定在单一节点，403/429 不再以同一毫秒批次无重试扩散到全部信源。
13. PostgreSQL 集成测试证明同一 schedule 最多存在一个未结束实例；外部依赖持续失败时，任务积压量不随调度轮次线性增长。
14. Worker Runtime 测试和线上 rollout 证明过期 `CLAIMED` 任务可在不重启 Pod 的情况下自动恢复，恢复数量通过指标和结构化日志可观测。
15. 线上代理状态展示总节点数、健康节点数、原节点索引和失败分类；Firecrawl 业务请求只使用通过 keyless 探测的节点，Bybit 无可用节点时明确降级且不再假报 ready。
