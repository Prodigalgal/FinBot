...
FinBot 的主要价值是研究分析，而不是依赖第三方服务完成网页抓取。本需求将 Firecrawl 从当前的主采集器降级为可选兜底，建立由 FinBot 自己控制请求、代理、解析、规范化、去重和可观测性的采集内核。

本阶段定义架构、契约、迁移门禁和验收标准，并按 changeset 逐步切换运行态。实现采用 breaking change，完成迁移后不保留以 `FIRECRAWL_*` 为核心的默认业务模式；旧 Firecrawl 枚举只作为显式 fallback 入口保留到影子门禁结束。
...
- Firecrawl 只作为显式配置的 fallback，具备熔断、预算和独立观测，不参与默认路径。
...
flowchart LR
    ...
    H --> O
    D -.超时/被阻断.-> P{是否允许兜底}
    P -->|是且未熔断| Q[FirecrawlFallbackAdapter]
    Q --> K
    P -->|否| R[明确失败]
```
...
| `FirecrawlFallbackAdapter` | 对明确标记为 JS-heavy 或被允许兜底的来源发起一次外部兜底 | 成为默认采集器或静默改变来源语义 |
...
| `FIRECRAWL_FALLBACK` | JS-heavy 或显式外部兜底 | Firecrawl adapter |

搜索发现与正文抓取保持独立：`SEARCH_DISCOVERY` 只返回带 canonical URL 的摘要证据；已知页面通过 `HTML_DOCUMENT` 另行抓取，通用搜索端点不可用不会阻断静态和官方来源。Firecrawl 只能作为显式 fallback。
...
- Firecrawl fallback 具备独立的每来源熔断器、日预算、最大调用次数和 `fallback_reason`；连续失败后进入冷却期。
- fallback 结果必须写明 `collector=firecrawl_fallback`，不能覆盖一方采集失败事实，也不能隐藏原始失败原因。
...
4. 代理路由、超时、大小、频率和并发。
5. 可选 Firecrawl fallback。
6. 保存前“测活并预览”，保存后显示最近采集阶段和失败原因。

Firecrawl 只作为“高级兜底”折叠项出现，不再作为信息源的主概念。

## 7. 迁移路线与门禁

### 阶段 0：影子比较

- 为现有静态 HTML、RSS、JSON 来源生成 first-party FetchPlan，但不写入证据。
- 同时记录与 Firecrawl 的标题、正文长度、canonical URL、内容哈希、延迟和失败分类差异。
- 通过 7 天样本建立每类来源的可用性基线；没有对比证据不得切换主路径。

### 阶段 1：一方协议接管

- RSS、JSON API、Sitemap 和静态 HTML 切换为 first-party primary。
- Firecrawl 仅保留在来源级 `fallback_policy=ALLOWED` 的 JS-heavy 页面。
- 研究下游只消费去重后的证据，不改变压缩和 AI 工作流。

### 阶段 2：搜索发现收敛

- 已知站点使用 sitemap、站内搜索模板或预置 URL 发现；通用互联网搜索作为显式可选能力。
- 将默认的 `FIRECRAWL_SEARCH` 和 `FIRECRAWL_SEARCH_THEN_SCRAPE` 迁移为 `SEARCH_DISCOVERY`；正文抓取由后续 `HTML_DOCUMENT` FetchPlan 独立执行。
- 迁移完成后禁止新建旧 Firecrawl source mode。

### 阶段 3：第三方依赖降级

- 连续 14 天 first-party primary 成功率、可用正文率、重复率和延迟达到门禁后，关闭默认 Firecrawl fallback。
- 再连续 14 天无必须依赖 Firecrawl 的来源，将 Firecrawl adapter 移到可独立部署/删除的扩展模块。
- 删除旧模式、旧配置键和旧生产资源前，完成历史数据只读回放验证和明确回滚点。
...
- Firecrawl fallback 关闭时，任何 first-party 失败都不隐式调用第三方。
...
- `FirecrawlSourceCollector` 不再被应用层依赖；fallback 可单独测试、熔断和移除。
- OpenAPI、TypeScript 客户端、Java 请求/响应对象和数据库 changeset 同步更新并进入 CI 契约检查。

## 9. 主要取舍

自研请求和解析能降低 Firecrawl 的可用性、费用和外部 IP 风控耦合，并让证据链、重试、代理和安全边界可控；代价是需要维护站点 profile、反爬差异和 HTML 解析质量。最稳妥的边界是：请求传输、静态页面提取、RSS/JSON/Sitemap、规范化和观测由 FinBot 自己负责；JavaScript 渲染和通用互联网搜索暂时保持可选 Firecrawl fallback，而不是在第一阶段引入浏览器集群。
