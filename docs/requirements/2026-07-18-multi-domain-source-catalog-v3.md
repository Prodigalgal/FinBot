...
默认目录由 Liquibase 046 以 append-only manifest 固化为 `v3`，包含 61 个稳定 source ID。manifest 只描述出厂目录；管理员仍可在 UI 中启停、修改或新增来源。
...
- 生产只部署一个内部单副本 `finbot-searxng`，通过不同来源的 `engines`、`categories`、`language` 和查询模板选择国内、国际或新闻引擎。
