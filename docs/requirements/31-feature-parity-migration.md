...
> 生产风险：截至 2026-07-15，Java 版已经完成运行时切换，但 Web 功能尚未达到旧版等价。生产写请求可稳定复现 CSRF 403；旧版 13 个业务工作区被压缩为 8 个页面，部分能力仅剩后端基础数据或完全没有 `/api/v2` 入口。本需求完成前不得再将“Java breaking migration”描述为用户功能迁移完成。
...
- 将新能力纳入同一基线：多工作流、节点级模型兜底、永久交易仓库、交易所启停、产品执行门禁和仅研究产品的预估交易。
...
| 分析与交易 | 市场分析 | 标的/交易所/周期输入、公共行情分析任务、进度和结构化结果 |
| 分析与交易 | 量化验证 | 杠杆/仓位预览、执行风控门禁、回测/统计结果、不可用原因 |
| 分析与交易 | 模拟账户 | 账户概览、区间盈亏、持仓、AI/风控/OMS/交易所统一时间线、模拟订单审计、预估交易 |
...
flowchart LR
  ...
  debate --> chair["主席裁决"]
  chair --> execution_review["最终执行机器人反思"]
...
| 产品库 | 丰富筛选、Watchlist、详情 | 基础筛选、详情、默认 Watchlist | 补自定义列表更新/删除、标签、排序、精度/最新行情和分页总量 |
| 市场分析 | 独立公共行情分析任务 | 没有用户入口 | 版本化 quant HTTP use case、异步 task/run、结果页 |
...
| 运行报告 | 多类结构化报告 | 没有报告工作区 | PostgreSQL projection API；摘要、指标、原始审计数据 |
...
- `/api/v2/analysis/market-runs`、`/api/v2/quant/previews`：市场分析和无副作用量化预览。
