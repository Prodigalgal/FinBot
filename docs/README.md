...
| 目录 | 内容 | 入口 |
| --- | --- | --- |
| `requirements/` | 业务边界、验收标准和按代码核验的状态 | [`README.md`](./requirements/README.md) |
| `decisions/` | 已接受的架构决策与取舍 | [`026-reusable-crawler-header-profiles.md`](./decisions/026-reusable-crawler-header-profiles.md) |
| `migrations/` | 数据与运行时迁移计划、门禁和回滚 | [`010-java-breaking-exec-plan.md`](./migrations/010-java-breaking-exec-plan.md) |
| `reports/` | 验收证据、审计和运行态结论 | [`31-crawler-c1-c2-production-status.md`](./reports/31-crawler-c1-c2-production-status.md) |
...
- 完成的 requirement 保留为契约快照，并在 [`requirements/README.md`](./requirements/README.md) 标记“已实现”；只有未闭环范围继续作为当前需求。
- 已发布 Liquibase changeset 不修改，只追加。
- 目录和命令统一以根 [`README.md`](../README.md) 为准；旧 Python 命令只允许出现在归档材料中。
- 任务状态维护在 [`tasks/current.md`](../tasks/current.md)；已完成任务移入 `tasks/done/`，不把历史流水账继续堆在 `in-progress/`。
...
- Browser Worker：[`services/browser-worker/README.md`](../services/browser-worker/README.md)
