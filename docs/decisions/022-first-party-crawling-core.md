...
一次性删除会把 JS 渲染和特殊站点迁移风险叠加在同一发布中。保留显式 fallback 可以让 first-party 在通过自动化测试和生产 smoke 后直接接管，同时不改变研究数据的真实性边界。
...
每个来源保留 `primaryCollector` 与 `fallbackPolicy` 版本。切换后发现正文质量或数据完整性回归时，只回滚该来源的 collector 配置到显式 Firecrawl fallback，不回滚数据库迁移、不删除 first-party 记录。删除旧 `SourceMode`、配置键和生产资源必须另开 breaking change 变更。
