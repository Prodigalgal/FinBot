...
Accepted，2026-07-18；浏览器渲染边界于 2026-07-19 被 [ADR 026](./026-reusable-crawler-header-profiles.md) 修订。first-party 在自动化测试和生产 smoke 通过后直接作为主路径，不设置按日历等待的影子比较门禁。
...
6. 第一阶段没有浏览器渲染；后续已经增加独立 `finbot-browser-worker`。C1 challenge 分类始终启用，C2/C3 只能由 Header Profile 显式开启，不得成为 Collector 的隐式 fallback。当前生产 Profile 全部关闭 bypass，详见 [运行态要求](../requirements/37-crawler-challenge-runtime.md)。
...
- Browser Worker 是独立无状态服务；每个 Playwright context 通过 `FINBOT_BROWSER_WORKER_PROXY_URL` 绑定 `WEB_CRAWL` proxy，NetworkPolicy 禁止直连。生产来源仍需完成受控 C2 smoke 后才可启用 Profile。
...
- 缺点：需要维护站点 Profile、挑战分类、浏览器镜像、代理隔离和更多测试；浏览器能力已部署，但只有完成来源级显式验收后才算运行可用。
