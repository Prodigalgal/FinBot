...
6. 在线测试不在控制面请求线程等待外部抓取；它使用独立 `source-test` 幂等域提交持久化 `INGESTION` 任务并立即返回 `202`。Web 通过任务短轮询读取终态，再以 source、原始 query 和任务创建时间定位本次 `source_collection_run`，保留原有获取、新增、重复和错误统计。
...
- 在线测试与手动采集共用同一采集内核和永久证据仓库，但幂等域、优先级和 UI 命令语义独立；浏览器刷新不会取消任务，超时后可在持久化后台任务中继续追踪。
...
- 公网在线测试应先返回 `202/PENDING`，随后任务进入终态并生成可按 source/query/时间关联的采集运行；普通 HTTP 请求不得再因搜索耗时返回 504。
