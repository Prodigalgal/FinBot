...
CONFIG_FIELD_SPECS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec("execution_robot.enabled", "模拟交易", "启用最终 AI 执行机器人", "boolean", True, help="在组合风险之后执行初审与反思终审；只能筛选既有决策，不能改变方向或订单规模。"),
    ConfigFieldSpec("execution_robot.max_output_tokens", "模拟交易", "执行机器人最大输出 Tokens", "integer", 2048, minimum=256, maximum=16384),
    ...
    ConfigFieldSpec("paper_execution.require_human_review", "模拟交易", "强制人工复核", "boolean", False, help="测试网默认由最终执行机器人自动批准；开启后改为仅接受人工批准的决策。"),
