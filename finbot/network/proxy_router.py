...
class ProxyRouter:
    ...
    def from_pools(
        ...
        firecrawl_proxy_ip_family: str = "unknown",
        firecrawl_dns_mode: str = "local",
        exchange_proxy_ip_family: str = "ipv6",
        ...
        return cls(
            {
                "firecrawl": _RouteConfig(
                    policy=ProxyRoutePolicy(
                        ...
                        require_proxy=True,
                        allow_direct=False,
                        proxy_ip_family=_clean_family(firecrawl_proxy_ip_family, "unknown"),
                        dns_mode=_clean_dns_mode(firecrawl_dns_mode),
                        allowed_ip_families=("ipv4", "ipv6", "dualstack", "unknown"),
                        description="Firecrawl must use a configured proxy pool and never fall back to direct.",
                    ),
                ...
                "exchange": _RouteConfig(
                    policy=ProxyRoutePolicy(
                        ...
                        proxy_ip_family=_clean_family(exchange_proxy_ip_family, "ipv6"),
