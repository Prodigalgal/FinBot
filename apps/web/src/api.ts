import type {
  ...
  SourceHealth, SourceMutation, SourceRecord, SourceTestResult, SystemSetting, TaskRecord, TradeAutomationConfiguration,
...
export const api = {
  ...
  testSource: (sourceId: string, queryText: string) => request<SourceTestResult>(`/api/v2/sources/${encodeURIComponent(sourceId)}/test`, { method: 'POST', body: JSON.stringify({ query: queryText }) }),
