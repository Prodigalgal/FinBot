...
export const api = {
  ...
  updateModel: (model: AiModel) => request<AiModel>(`/api/v2/configuration/models/${encodeURIComponent(model.modelProfileId)}`, { method: 'PUT', body: JSON.stringify({ defaultReasoningEffort: model.defaultReasoningEffort, maximumReasoningEffort: model.maximumReasoningEffort, tokenLimitParameterStyle: model.tokenLimitParameterStyle, inputUsdPerMillion: model.inputUsdPerMillion, outputUsdPerMillion: model.outputUsdPerMillion, enabled: model.enabled, expectedVersion: model.version }) }),
  ...
  createCrawlerHeaderProfile: (body: CrawlerHeaderProfileMutation) => request<CrawlerHeaderProfile>('/api/v2/crawler/header-profiles', { method: 'POST', body: JSON.stringify(body) }),
  updateCrawlerHeaderProfile: (profileId: string, expectedVersion: number, body: CrawlerHeaderProfileMutation) => request<CrawlerHeaderProfile>(`/api/v2/crawler/header-profiles/${encodeURIComponent(profileId)}${query({ expectedVersion })}`, { method: 'PUT', body: JSON.stringify(body) }),
  deleteCrawlerHeaderProfile: (profileId: string, expectedVersion: number) => request<void>(`/api/v2/crawler/header-profiles/${encodeURIComponent(profileId)}${query({ expectedVersion })}`, { method: 'DELETE' }),
  createSource: (body: SourceMutation) => request<SourceRecord>('/api/v2/sources', { method: 'POST', body: JSON.stringify(body) }),
  updateSource: (sourceId: string, expectedVersion: number, body: SourceMutation) => request<SourceRecord>(`/api/v2/sources/${encodeURIComponent(sourceId)}${query({ expectedVersion })}`, { method: 'PUT', body: JSON.stringify(body) }),
  deleteSource: (sourceId: string, expectedVersion: number) => request<void>(`/api/v2/sources/${encodeURIComponent(sourceId)}${query({ expectedVersion })}`, { method: 'DELETE' }),
