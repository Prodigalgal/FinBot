import type {
  ...
  ResearchFeedback, ResearchForecast, ResearchHistoryDetail, ResearchLaunch, ResearchSummary, RiskPolicy,
...
export const api = {
  ...
  marketAnalysis: (body: { instrumentId: string; symbol: string; exchange: string; intervalSeconds: number; forecastHorizonSeconds: number; question: string; workflowVersionId: string | null }, key: string) => request<ResearchLaunch>('/api/v2/analysis/market-runs', { method: 'POST', headers: idempotency(key), body: JSON.stringify(body) }),
