...
import { api } from './api';
import type { AiModel, AiModelBinding, AiProvider, ConfigurationSnapshot, ReasoningEffort, WorkflowDefinitionSummary, WorkflowEdge, WorkflowNode, WorkflowSchema, WorkflowVersion } from './types';
...
export function WorkflowPage() {
  ...
  return <Stack spacing={2}>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.25} alignItems={{ md: 'center' }}><TextField select label="工作流" value={definition.definitionId} onChange={(event) => void selectDefinition(event.target.value)} sx={{ minWidth: 260 }}>{definitions.map((item) => <MenuItem key={item.definitionId} value={item.definitionId}>{item.name}{item.active ? '（调度中）' : ''}</MenuItem>)}</TextField><TextField label="辩论轮次" type="number" value={version.defaultDebateRounds} onChange={(event) => setVersion({ ...version, defaultDebateRounds: Number(event.target.value) })} inputProps={{ min: 1, max: 8 }} sx={{ width: 120 }} /><Chip label={`${version.status} v${version.versionNumber}`} color={version.status === 'PUBLISHED' ? 'success' : 'warning'} /><FormControlLabel control={<Switch checked={definition.active} disabled={saving || definition.publishedVersionId === null} onChange={(event) => void setActive(event.target.checked)} />} label="参与定时调度" /><Box sx={{ flex: 1 }} /><Button startIcon={<AddIcon />} onClick={addAgent}>添加角色</Button><Button variant="contained" startIcon={<SaveIcon />} disabled={saving} onClick={() => void save()}>保存草稿</Button><Button color="success" variant="contained" startIcon={<PublishIcon />} disabled={saving || version.status !== 'DRAFT'} onClick={() => void publish()}>发布</Button></Stack>
...
function NodeEditor({ node, schema, providers, models, update, remove }: { node: WorkflowNode; schema: WorkflowSchema; providers: AiProvider[]; models: AiModel[]; update: (patch: Partial<WorkflowNode>) => void; remove: () => void }) {
  ...
  const changeNodeType = (nodeType: string) => {
    if (!isLlmBacked(nodeType)) {
      update({ nodeType, roleName: null, primaryAiBinding: null, fallbackAiBinding: null, systemPrompt: null, userPromptTemplate: null, outputContract: null });
      return;
    }
    ...
    update({
      ...
      primaryAiBinding,
      systemPrompt: node.systemPrompt || '你是专业市场研究角色。基于上游证据进行分析，只输出工作流要求的严格 JSON。',
      userPromptTemplate: node.userPromptTemplate || '分析上游研究证据，给出可验证结论、反例与修订。',
      outputContract: node.outputContract || 'DEBATE_ARGUMENT',
    });
  };
