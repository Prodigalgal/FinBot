...
export function CatalogPage({ onResearch }: { onResearch?: (question: string) => void }) {
  ...
  return <Stack spacing={2.5}>
    ...
    {detail && <Dialog open maxWidth="md" fullWidth onClose={() => setDetail(null)}><DialogTitle>{detail.displayName}</DialogTitle><DialogContent dividers><Stack spacing={1.5}>{detail.instruments.map((instrument) => <Paper key={instrument.instrumentId} variant="outlined" sx={{ p: 1.5 }}><Stack direction="row" justifyContent="space-between"><Typography fontWeight={700}>{instrument.exchange} · {instrument.symbol}</Typography><Chip size="small" label={`${instrument.maximumLeverage}x max`} /></Stack><Typography variant="body2" color="text.secondary">{instrument.marketType} · 合约单位 {instrument.contractSize} · 价格步长 {instrument.priceTick} · 数量步长 {instrument.quantityStep} · 最小数量 {instrument.minimumQuantity}</Typography></Paper>)}</Stack></DialogContent><DialogActions><Button onClick={() => setDetail(null)}>关闭</Button><Button variant="contained" onClick={() => { onResearch?.(`对 ${detail.displayName} 进行完整研究与模拟交易评估`); setDetail(null); }}>发起研究</Button></DialogActions></Dialog>}
