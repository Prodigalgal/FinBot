...
function SourceEditorDialog({ open, source, busy, onClose, onSave }: { open: boolean; source: SourceRecord | null; busy: boolean; onClose: () => void; onSave: (source: SourceMutation) => Promise<void> }) {
  ...
  const [showFirecrawlFallback, setShowFirecrawlFallback] = useState(false);
  useEffect(() => { if (open) { setDraft(source ? sourceMutation(source) : DEFAULT_SOURCE); setShowFirecrawlFallback(Boolean(source?.mode.startsWith('FIRECRAWL'))); } }, [open, source]);
  ...
  const sourceModes = ['RSS', 'HTML_DOCUMENT', 'SEARCH_DISCOVERY', 'JSON_API', 'SITEMAP', 'EXCHANGE_PUBLIC_API', ...(showFirecrawlFallback ? ['FIRECRAWL_SCRAPE', 'FIRECRAWL_SEARCH', 'FIRECRAWL_SEARCH_THEN_SCRAPE'] : [])];
  return <Dialog open={open} onClose={busy ? undefined : onClose} maxWidth="md" fullWidth><DialogTitle>{source ? '编辑信息源' : '新增信息源'}</DialogTitle><DialogContent dividers><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' }, gap: 1.5, pt: .5 }}>
    ...
    <Stack direction="row" spacing={2} useFlexGap flexWrap="wrap" sx={{ gridColumn: { sm: '1 / -1' } }}><FormControlLabel control={<Switch checked={draft.credentialSupported} onChange={(event) => set('credentialSupported', event.target.checked)} />} label="需要 API Key" /><FormControlLabel control={<Switch checked={draft.enabled} onChange={(event) => set('enabled', event.target.checked)} />} label="创建后启用" /><FormControlLabel control={<Switch checked={showFirecrawlFallback} onChange={(event) => setShowFirecrawlFallback(event.target.checked)} />} label="显示 Firecrawl 高级兜底类型" /></Stack>
