...
function SourceEditorDialog({ open, source, busy, onClose, onSave }: { open: boolean; source: SourceRecord | null; busy: boolean; onClose: () => void; onSave: (source: SourceMutation) => Promise<void> }) {
  ...
  useEffect(() => { if (open) setDraft(source ? sourceMutation(source) : DEFAULT_SOURCE); }, [open, source]);
  ...
  return <Dialog open={open} onClose={busy ? undefined : onClose} maxWidth="md" fullWidth><DialogTitle>{source ? '编辑信息源' : '新增信息源'}</DialogTitle><DialogContent dividers><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' }, gap: 1.5, pt: .5 }}>
    ...
    <TextField select label="类型" value={draft.mode} onChange={(event) => { const mode = event.target.value; setDraft((current) => ({ ...current, mode, outboundRoute: mode.startsWith('FIRECRAWL') ? 'FIRECRAWL' : 'PUBLIC_DATA' })); }}>{['RSS', 'FIRECRAWL_SCRAPE', 'FIRECRAWL_SEARCH', 'FIRECRAWL_SEARCH_THEN_SCRAPE', 'STRUCTURED_API', 'EXCHANGE_PUBLIC_API'].map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField>
    ...
    <TextField select label="出站路由" value={draft.outboundRoute || ''} onChange={(event) => set('outboundRoute', event.target.value || null)} disabled={firecrawl}>{['PUBLIC_DATA', 'FIRECRAWL', 'EXCHANGE_GATE', 'EXCHANGE_BYBIT'].map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField>
    <TextField type="url" label="Endpoint Base URL" value={draft.endpointBaseUrl || ''} onChange={(event) => set('endpointBaseUrl', event.target.value || null)} required={draft.mode !== 'RSS'} />
    ...
    <TextField label="Seed URL（每行一个）" value={draft.seedUrls.join('\n')} onChange={(event) => set('seedUrls', splitLines(event.target.value))} multiline minRows={2} required={draft.mode === 'FIRECRAWL_SCRAPE'} />
    ...
    <Stack direction="row" spacing={2} sx={{ gridColumn: { sm: '1 / -1' } }}><FormControlLabel control={<Switch checked={draft.credentialSupported} onChange={(event) => set('credentialSupported', event.target.checked)} />} label="需要 API Key" /><FormControlLabel control={<Switch checked={draft.enabled} onChange={(event) => set('enabled', event.target.checked)} />} label="创建后启用" /></Stack>
...
function sourceDraftValid(source: SourceMutation): boolean { if (!source.displayName.trim() || !source.category.trim()) return false; if (source.mode === 'RSS') return source.feedUrls.length > 0; if (!source.endpointBaseUrl) return false; if (source.mode === 'FIRECRAWL_SCRAPE') return source.seedUrls.length > 0; return true; }
