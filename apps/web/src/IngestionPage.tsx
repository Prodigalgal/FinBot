...
export function IngestionPage() {
  ...
  const [sourceEnabled, setSourceEnabledFilter] = useState('');
  ...
  const filteredSources = useMemo(() => {
    ...
    return workspace?.sources.filter((source) => {
      const definition = sourceCatalog.find((candidate) => candidate.sourceId === source.sourceId);
      const matchesText = !normalized || [source.displayName, source.sourceId, source.category, definition?.provider || '']
        .some((value) => value.toLowerCase().includes(normalized));
      return matchesText
        && (!sourceCategory || source.category === sourceCategory)
        && (!sourceMode || source.mode === sourceMode)
        && (!sourceEnabled || source.enabled === (sourceEnabled === 'enabled'));
    }) || [];
  }, [sourceCatalog, sourceCategory, sourceEnabled, sourceFilter, sourceMode, workspace]);
  const sourceCategories = useMemo(() => [...new Set(workspace?.sources.map((source) => source.category) || [])].sort(), [workspace]);
  const sourceModes = useMemo(() => [...new Set(workspace?.sources.map((source) => source.mode) || [])].sort(), [workspace]);
  ...
  return <Stack spacing={3}>
    ...
    <Box><SectionTitle title={`信息源状态（${filteredSources.length}/${workspace.sources.length}）`} action={<Button startIcon={<AddIcon />} onClick={() => setEditorSource('new')}>新增信源</Button>} />
      <Paper variant="outlined" sx={{ p: 1.5, mb: 1 }}><Stack direction={{ xs: 'column', md: 'row' }} spacing={1.25}>
        <TextField size="small" label="搜索信源" value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value)} sx={{ flex: 1, minWidth: 220 }} />
        <TextField select size="small" label="分类" value={sourceCategory} onChange={(event) => setSourceCategory(event.target.value)} sx={{ minWidth: 190 }}><MenuItem value="">全部</MenuItem>{sourceCategories.map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField>
...
function SourceEditorDialog({ open, source, providers, models, busy, onClose, onSave }: { open: boolean; source: SourceRecord | null; providers: AiProvider[]; models: AiModel[]; busy: boolean; onClose: () => void; onSave: (source: SourceMutation) => Promise<void> }) {
  ...
  const aiWebSearch = draft.mode === 'AI_WEB_SEARCH';
  const sourceModes = ['RSS', 'HTML_DOCUMENT', 'SEARCH_DISCOVERY', 'JSON_API', 'SITEMAP', 'AI_WEB_SEARCH', 'EXCHANGE_PUBLIC_API', ...(showFirecrawlChannel ? ['FIRECRAWL_SCRAPE', 'FIRECRAWL_SEARCH', 'FIRECRAWL_SEARCH_THEN_SCRAPE'] : [])];
  const enabledProviders = providers.filter((provider) => provider.enabled && models.some((model) => model.enabled && model.providerProfileId === provider.profileId));
  const providerModels = models.filter((model) => model.enabled && model.providerProfileId === draft.aiWebSearchBinding?.providerProfileId);
  const changeMode = (mode: string) => {
    const preferredProvider = enabledProviders.find((provider) => provider.profileId.includes('grok')) || enabledProviders[0];
    const preferredModel = models.find((model) => model.enabled && model.providerProfileId === preferredProvider?.profileId);
    setDraft((current) => ({
      ...current,
      mode,
      provider: mode === 'AI_WEB_SEARCH' ? 'ai_web_search' : current.provider === 'ai_web_search' ? null : current.provider,
      outboundRoute: mode === 'AI_WEB_SEARCH' ? null : mode === 'HTML_DOCUMENT' || mode === 'SEARCH_DISCOVERY' ? 'WEB_CRAWL' : mode.startsWith('FIRECRAWL') ? 'FIRECRAWL' : 'PUBLIC_DATA',
      endpointBaseUrl: mode === 'AI_WEB_SEARCH' ? null : current.endpointBaseUrl,
      credentialSupported: mode === 'AI_WEB_SEARCH' ? false : current.credentialSupported,
      aiWebSearchBinding: mode === 'AI_WEB_SEARCH' && preferredProvider && preferredModel ? {
        providerProfileId: preferredProvider.profileId,
        modelName: preferredModel.modelName,
        reasoningEffort: preferredModel.defaultReasoningEffort,
        tool: preferredProvider.profileId.includes('gemini') ? 'GOOGLE_SEARCH' : 'WEB_SEARCH',
      } : null,
    }));
  };
  return <Dialog open={open} onClose={busy ? undefined : onClose} maxWidth="md" fullWidth><DialogTitle>{source ? '编辑信息源' : '新增信息源'}</DialogTitle><DialogContent dividers><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' }, gap: 1.5, pt: .5 }}>
    ...
    <TextField select label="类型" value={draft.mode} onChange={(event) => changeMode(event.target.value)}>{sourceModes.map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField>
    ...
    <TextField label="Provider 标识" value={draft.provider || ''} onChange={(event) => set('provider', event.target.value || null)} inputProps={{ maxLength: 80 }} disabled={aiWebSearch} />
    ...
    {aiWebSearch && <><TextField select label="AI 厂商" value={draft.aiWebSearchBinding?.providerProfileId || ''} onChange={(event) => { const providerProfileId = event.target.value; const model = models.find((candidate) => candidate.enabled && candidate.providerProfileId === providerProfileId); const provider = providers.find((candidate) => candidate.profileId === providerProfileId); set('aiWebSearchBinding', model ? { providerProfileId, modelName: model.modelName, reasoningEffort: model.defaultReasoningEffort, tool: provider?.profileId.includes('gemini') ? 'GOOGLE_SEARCH' : 'WEB_SEARCH' } : null); }}>{enabledProviders.map((provider) => <MenuItem key={provider.profileId} value={provider.profileId}>{provider.displayName}</MenuItem>)}</TextField>
      ...
      <TextField select label="思考强度" value={draft.aiWebSearchBinding?.reasoningEffort || 'PROVIDER_DEFAULT'} onChange={(event) => { if (draft.aiWebSearchBinding) set('aiWebSearchBinding', { ...draft.aiWebSearchBinding, reasoningEffort: event.target.value as ReasoningEffort }); }}>{REASONING_EFFORTS.map((effort) => <MenuItem key={effort} value={effort}>{effort}</MenuItem>)}</TextField>
      <TextField select label="搜索工具" value={draft.aiWebSearchBinding?.tool || 'WEB_SEARCH'} onChange={(event) => { if (draft.aiWebSearchBinding) set('aiWebSearchBinding', { ...draft.aiWebSearchBinding, tool: event.target.value as 'WEB_SEARCH' | 'GOOGLE_SEARCH' }); }}><MenuItem value="WEB_SEARCH">WEB_SEARCH</MenuItem><MenuItem value="GOOGLE_SEARCH">GOOGLE_SEARCH</MenuItem></TextField></>}
    ...
    <TextField label="搜索查询（每行一个）" value={draft.searchQueries.join('\n')} onChange={(event) => set('searchQueries', splitLines(event.target.value))} multiline minRows={2} required={aiWebSearch} />
    <Stack direction="row" spacing={2} useFlexGap flexWrap="wrap" sx={{ gridColumn: { sm: '1 / -1' } }}><FormControlLabel control={<Switch checked={draft.credentialSupported} disabled={aiWebSearch} onChange={(event) => set('credentialSupported', event.target.checked)} />} label="需要 API Key" /><FormControlLabel control={<Switch checked={draft.enabled} onChange={(event) => set('enabled', event.target.checked)} />} label="创建后启用" /><FormControlLabel control={<Switch checked={showFirecrawlChannel} onChange={(event) => setShowFirecrawlChannel(event.target.checked)} />} label="显示 Firecrawl 渠道" /></Stack>
...
function sourceDraftValid(source: SourceMutation): boolean { if (!source.displayName.trim() || !source.category.trim()) return false; if (source.mode === 'RSS') return source.feedUrls.length > 0; if (source.mode === 'HTML_DOCUMENT') return source.seedUrls.length > 0; if (source.mode === 'AI_WEB_SEARCH') return source.aiWebSearchBinding !== null && source.searchQueries.length > 0; if (!source.endpointBaseUrl) return false; if (source.mode === 'FIRECRAWL_SCRAPE') return source.seedUrls.length > 0; return true; }
