...
import { EmptyBlock, ErrorBlock, LoadingBlock, MetricStrip, SectionTitle, StatusBadge, CopyableText, formatMoney, formatTime, statusLabel } from './ui';
...
export function TradingActivityPanel({ accounts }: { accounts: AccountOverview[] }) {
  ...
  return <Stack spacing={2.25}>
    ...
    <Paper variant="outlined" sx={{ p: 2, boxShadow: '0 1px 3px rgba(15, 23, 42, 0.04)' }}><Stack spacing={1.75}><Stack direction="row" spacing={1} alignItems="center"><FilterAltOutlinedIcon color="primary" fontSize="small" /><Typography fontWeight={700} variant="body2">操作历史精准检索</Typography></Stack><Stack direction={{ xs: 'column', md: 'row' }} spacing={1.25} flexWrap={{ md: 'wrap' }} useFlexGap>
      ...
      <Button variant="contained" onClick={apply} disabled={loading} sx={{ minWidth: 100 }}>应用筛选</Button>
    </Stack></Stack></Paper>

    {loading && !payload ? <LoadingBlock label="正在读取永久交易仓库" /> : payload && <>
      <MetricStrip items={[
        { label: '匹配记录', value: payload.matchedCount.toLocaleString() },
        { label: 'AI 决策', value: count(payload, 'DECISION').toLocaleString() },
        { label: '风险 / 预估', value: (count(payload, 'RISK_ASSESSMENT') + count(payload, 'ESTIMATE')).toLocaleString() },
        { label: '本地订单事件', value: (count(payload, 'OMS_ORDER') + count(payload, 'OMS_EVENT')).toLocaleString() },
        { label: '交易所订单 / 成交', value: (count(payload, 'ORDER') + count(payload, 'FILL')).toLocaleString() },
      ]} />

      <Box><SectionTitle title="数据来源与完整性" subtitle="多渠道订单、成交与持仓同步对账状态" /><Paper variant="outlined" sx={{ overflow: 'hidden' }}>{payload.sources.map((source, index) => <Stack key={`${source.source}-${source.accountId || 'local'}`} direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1} sx={{ px: 2, py: 1.35, borderTop: index ? '1px solid' : 0, borderColor: 'divider', transition: 'background-color 0.12s ease', '&:hover': { bgcolor: 'action.hover' } }}><Box><Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap"><Typography variant="body2" fontWeight={700}>{source.source === 'LOCAL_OMS' ? 'FinBot 本地永久审计' : `${source.exchange} · ${source.accountId}`}</Typography><StatusBadge status={source.status} /><Chip size="small" variant="outlined" label={source.complete ? '来源完整' : '需要关注'} /></Stack><Typography variant="caption" color="text.secondary">{source.message}</Typography></Box><Typography variant="caption" color="text.secondary" sx={{ whiteSpace: 'nowrap' }}>最新 {formatTime(source.latestAt)}</Typography></Stack>)}</Paper></Box>

      <Box><SectionTitle title="统一操作时间线" subtitle="按时间倒序排列的全局不可篡改事实流" />{desktop ? <DesktopActivityTable payload={payload} expanded={expanded} setExpanded={setExpanded} /> : <MobileActivityList payload={payload} expanded={expanded} setExpanded={setExpanded} />}{payload.activities.length === 0 && <Paper variant="outlined"><EmptyBlock>{payload.matchedCount === 0 ? '当前筛选条件下确实没有操作记录' : '本页没有可显示记录'}</EmptyBlock></Paper>}</Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center"><Typography variant="caption" color="text.secondary">第 {history.length + 1} 页 · 当前 {payload.activities.length} 条 / 共 {payload.matchedCount} 条</Typography><Stack direction="row" spacing={1}><Button variant="outlined" size="small" onClick={previous} disabled={loading || history.length === 0}>上一页</Button><Button variant="outlined" size="small" onClick={next} disabled={loading || !payload.nextCursor}>下一页</Button></Stack></Stack>
    </>}
  </Stack>;
}
...
function ActivityDetail({ activity }: { activity: ActivityRecord }) {
  ...
  return <Box sx={{ my: 1, p: 2, bgcolor: 'action.hover', borderRadius: '4px', border: '1px solid', borderColor: 'divider' }}><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))', xl: 'repeat(3, minmax(0, 1fr))' }, gap: 1.5 }}>{facts.map(([label, value], index) => <Box key={`${label}-${index}`} sx={{ minWidth: 0 }}><Typography variant="caption" color="text.secondary" sx={{ textTransform: 'uppercase', letterSpacing: '0.02em', fontSize: 10.5 }}>{detailLabel(label)}</Typography><Typography variant="body2" sx={{ overflowWrap: 'anywhere', whiteSpace: 'pre-wrap', fontWeight: 600, mt: 0.25 }}>{detailValue(value)}</Typography></Box>)}</Box></Box>;
}
