...
SET system_prompt = CASE stage
    ...
    user_prompt_template = CASE stage
        ...
        WHEN 'REFLECTION' THEN '严格输出字段 verdict、reasons、decision。verdict 只能是 APPROVE 或 REJECT，reasons 必须是非空字符串数组。REJECT 时 decision 必须为 null；APPROVE 时 decision 必须是完整对象，严格复用初审的八个字段、类型和 HOLD/WATCH null 规则，不得增加其他字段。'
    END,
    version = version + 1,
