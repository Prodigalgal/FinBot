...
- 状态：first-party 核心已完成并在上一版本生产运行；免费结构化目录 v2 已在源码和 CI 门禁落地，尚待发布到生产（2026-07-18）。
- 目标：由 FinBot 自己完成请求、代理路由、静态协议解析、抽取、规范化和观测；Firecrawl 与 first-party 是独立渠道，默认关闭 Firecrawl，不能作为 first-party 失败后的隐式 fallback。
- 已完成：固定 11 个 v1 默认来源并追加 Liquibase 044 v2（SEC/GDELT/World Bank/BLS/CFTC/FRED/EIA 目录策略）；manifest 保留历史并按最新创建时间读取当前版本；`HTML_DOCUMENT`、`RSS`、`JSON_API`、`SITEMAP` first-party collector；`WEB_CRAWL` 独立路由、Deployment、Service 与 `proxygateway_web_crawl` profile；Firecrawl profile 固定私有四节点 `INLINE_NODES` 但默认 disabled；搜索发现 `SEARCH_DISCOVERY`（GDELT/SearXNG/Brave 兼容 JSON）并拆出 `SearchDiscoveryProvider`；统一 `CrawlerTransport`；ContentEnvelope/稳定 `blockId` 安全视图；`normalized_document.content_blocks` 与多 Agent 合法 block 引用门禁；全局/来源/单主机背压与主机礼貌延迟；SSRF（配置与运行时 DNS）拒绝、代理 fail-closed、每请求换出口、重试/重定向错误码与凭据隔离；append-only `source_fetch_attempt`（含 redirect_count）；来源运行健康 API、Firecrawl 独立预算/熔断、采集中断恢复；相关 Java/Web/OpenAPI/Proxy 测试。
- 当前边界：上一生产版本已完成真实 PostgreSQL、K8S、ArgoCD 与代理控制面 smoke；本次 v2 尚未写入生产数据库。Firecrawl 私有四节点当前 `0/4` 健康并保持 fail-closed，不能伪装为可用。GDELT/SEC 的上游限流或出口阻断会显示为明确的 `429/403`，不会被伪装成成功；第一阶段不引入浏览器渲染，不改变研究和交易工作流。
