...
实现阶段：first-party 协议、ContentEnvelope、生产控制面和目录 v1/v2/v3 已落地，国内外新闻、内部 SearXNG 与 AI Web Search 已生产发布；当前追加 Liquibase 047，将来源级引擎配置编译为 SearXNG 官方支持的 `!shortcut` 查询语法；不设置 14 天影子比较门禁。
...
1. 在 CI PostgreSQL 服务中执行 Liquibase 047 完整升级门禁，断言 6 个 SearXNG 来源使用 `engine_shortcuts` 且遗留 `engines` 为 0。
2. 发布到 K8S/ArgoCD，验证 047 已登记、Backend 新版本 Ready、NetworkPolicy 与代理健康。
3. 执行国内、国际和新闻 engine shortcut 的真实 JSON 搜索 smoke；GDELT/SEC 或搜索引擎的 429/403 必须以健康状态和 attempt 记录呈现。
