...
实现阶段：first-party 协议、ContentEnvelope、生产控制面和目录 v1/v2 已落地；多领域目录 v3、国内外新闻、内部 SearXNG 与 AI Web Search 正在执行全量门禁和生产发布；不设置 14 天影子比较门禁。
...
1. 在 CI PostgreSQL 服务中执行 Liquibase v3 完整升级门禁，覆盖 append-only manifest、AI binding/audit 和 61 个默认来源字段。
2. 发布到 K8S/ArgoCD，验证 `finbot-searxng` 单副本 Ready、NetworkPolicy、代理健康和至少一次真实 JSON 搜索结果。
3. 执行国内、国际、垂直行业、交易所和 AI 搜索来源 smoke；GDELT/SEC 或搜索引擎的 429/403 必须以健康状态和 attempt 记录呈现。
