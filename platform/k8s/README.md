...
当前生产环境按成本和集群容量选择单副本：Backend、Quant、Web、Firecrawl Proxy 和 Exchange Proxy 的 `replicas` 均为 `1`，PostgreSQL 为单 StatefulSet Pod。系统不声明高可用，发布期间允许短暂中断；受集群容量限制的工作负载使用 `maxSurge: 0` 避免 rollout 永久等待新 Pod 调度。
