...
> 当前清单是 Java/Quant/Web/Browser Worker 稳态生产拓扑。旧 `Deployment/finbot` 与一次性 SQLite 导入 Hook 已在历史迁移核验完成后退役；`finbot-state` PVC 仅作为受控回滚证据保留。不要绕过 ArgoCD 长期维护生产资源。
...
`FIRECRAWL_PROXY_NODES` 与 `EXCHANGE_PROXY_NODES` 使用分号分隔 VLESS/Hysteria2 URL；Firecrawl 只允许使用私有四节点池，禁止回退到大订阅池，且默认 profile/source 均关闭。Firecrawl proxy 以空载控制面启动，不会因为 Secret 中存在节点而自动探测；管理员显式开启 profile 后，Java 才会热加载四节点。`WEB_CRAWL_SUBSCRIPTION_URL` 由独立网页采集网关轮换。`SEARXNG_SECRET` 是内部 SearXNG 实例密钥，至少 32 个随机字符；SearXNG 不对公网暴露，搜索请求强制经 `finbot-web-crawl-proxy`。三套 Proxy Gateway 使用同一双内核镜像，每个 Profile 独立选择 `SING_BOX` 或 `XRAY` 并维护目标感知健康状态；Xray 只接受 VLESS，HY2 必须使用 sing-box。Java 的 `WEB_CRAWL`、`FIRECRAWL` 和交易路由不会共享健康状态。Firecrawl、网页采集与 SearXNG 均 fail-closed；交易所是否允许直连由 `network_proxy_route.allow_direct` 决定。

`finbot-browser-worker` 是独立 C2 Playwright 服务，只接受 Backend Pod 携带内部 Bearer token 的请求。Chromium 每个 context 都注入 `FINBOT_BROWSER_WORKER_PROXY_URL`，没有直连回退；NetworkPolicy 仅允许 DNS 和到 `finbot-web-crawl-proxy:8080` 的 egress。当前生产来源仍未启用 C2，启用前必须执行受控的代理出口、challenge、cookie/UA 回放和关闭回滚 smoke。
...
| `-10` | 三个 proxy gateway、SearXNG | 代理控制面与 SearXNG 进程 Ready；Firecrawl 默认 `enabled=false`，SearXNG 搜索结果另做 smoke |
| `0` | Java、Quant、Web、Browser Worker | 所有 Deployment 与探针 Ready；Browser Worker health 证明 Chromium 和强制代理配置就绪，不代表任一 challenge 已成功处理 |
...
当前生产环境按成本和集群容量选择单副本：Backend、Quant、Web、Browser Worker、SearXNG 和三个 Proxy Gateway 的 `replicas` 均为 `1`，PostgreSQL 为单 StatefulSet Pod，稳态共 9 个 Pod。Firecrawl 默认空载，不会因启动环境中的节点列表主动访问上游。系统不声明高可用，发布期间允许短暂中断；所有单副本 Deployment 统一使用 `maxSurge: 0` 和 `maxUnavailable: 1`，避免 rollout 因集群容量不足永久等待新 Pod 调度。PostgreSQL、bootstrap Job 与 SearXNG 使用固定 ARM64 image digest。
...
Core Actions 同步本目录并更新 Backend、Quant、Web、Browser Worker 四个 tag；Proxy Actions 只更新独立代理镜像 tag。两个流水线通过 GitOps concurrency 串行写入私有仓库 `Prodigalgal/ircs-prod-config/finbot`。ArgoCD Application `finbot` 从 `finbot/oracle` 自动同步；CI 不持有 kubeconfig，也不直接写集群。
...
kubectl -n finbot rollout status deployment/finbot-searxng --timeout=5m
kubectl -n finbot exec deployment/finbot-browser-worker -- python -c "import json,urllib.request; print(json.load(urllib.request.urlopen('http://127.0.0.1:8082/internal/v1/health', timeout=5)))"
```
...
`/healthz` 只检查 SearXNG Web 进程。必须从 Backend Pod 内执行结果级 smoke，确认 NetworkPolicy、DNS、代理和至少一个搜索引擎同时可用：

```powershell
$backendPod = kubectl -n finbot get pod -l app.kubernetes.io/name=finbot-backend -o jsonpath='{.items[0].metadata.name}'
kubectl -n finbot exec $backendPod -- curl -fsS --get `
  'http://finbot-searxng:8080/search' `
  --data-urlencode 'q=全球科技与金融最新新闻' `
  --data-urlencode 'categories=general,news' `
  --data-urlencode 'language=zh-CN' `
  --data-urlencode 'format=json'
```

响应必须为 JSON object，`results` 为数组且至少包含一个合法公网 HTTP(S) URL；空结果、引擎错误或代理无健康出口不能仅凭 Pod Ready 判定上线成功。
